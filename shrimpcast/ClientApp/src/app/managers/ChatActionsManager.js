class ChatActionsManager {
  ...
  static async GetMessageInfo(signalR, messageId, sessionId, useSession) {
    const response = await signalR.invoke('GetInformation', messageId || 0, useSession ? sessionId : 0).catch(ex => console.log(ex));
    return response;
  }
  ...
  static async TriggerCountChange(signalR) {
    await signalR.invoke('TriggerUserCountChange', true).catch(ex => console.log(ex));
  }
}
